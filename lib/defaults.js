(function (root, factory) {
    ...
    var defaults = function (schema, definitions) {
        ...
        // Check the `schema.default` property
        if (typeof schema['default'] !== 'undefined') {
            ...
            if (!schema.properties) {
                // wiwo return empty object that will be excluded
                return undefined;
            }
            ...
            for (var key in schema.properties) {
                if (schema.properties.hasOwnProperty(key)) {
                    const sub = defaults(schema.properties[key], definitions);
                    if (sub !== undefined){
                        provisionalResult[key] = sub;
                        validCount++;
                    }
                }
            }
            ...
            if (Array.isArray(schema.items)) {
                schema.items.forEach(function (item) {
                    const curValue = defaults(item, definitions);
                ...
                if (typeof value !== 'undefined') {
                    // wiwo: Return empty array if minItems is 0
            ...
            if (values.length){
                // wiwo: Allow default array if there are items in it
                return values;
            } else {
                // wiwo: Exclude the default if the array is empty
                return undefined;
            }
        }
